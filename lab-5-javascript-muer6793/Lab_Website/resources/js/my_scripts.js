 ...
 */
 ...
 function loadStatsPage(){
  ...
  for(var row = 2; row < table1.rows.length; row++){
    if(table1.rows[row].cells[2].innerHTML > table1.rows[row].cells[3].innerHTML){
      Ws++;
      table1.rows[row].cells[4].innerHTML = "Buffs"
      console.log(Ws)

    }
    else{
      Ls++
      table1.rows[row].cells[4].innerHTML = table1.rows[row].cells[1].innerHTML
    }
  }
  var table2wins = document.getElementById("wins");
  var table2losses = document.getElementById("losses");
  table2wins.innerHTML = Ws;
  table2losses.innerHTML = Ls;
 }
 ...
 */
 ...
 function loadPlayersPage()
 {
    ...
    for(var i = 0; i<players.length;i++)
    {
        ...
        menu.innerHTML += '<a class = "dropdown-item" href="#" onclick = "switchPlayers(\''+ i +'\')">' + name +"</a>"
    }
 }
...
function switchPlayers(index){
  ...
  document.getElementById("avg_p_yards").innerHTML = parseInt(players[index].pass_yards/players[index].games_played);
  document.getElementById("avg_r_yards").innerHTML = parseInt(players[index].rushing_yards/players[index].games_played);
  document.getElementById("avg_rec_yards").innerHTML = parseInt(players[index].receiving_yards/players[index].games_played);
  var pictureIndex = parseInt[index]+1;
  console.log(pictureIndex);
  document.getElementById("player_img").src = "/resources/img/player" + pictureIndex + ".jpg";
}
